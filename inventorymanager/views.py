...
def index(request):
    ...
    if request.user.is_authenticated:
        ...
        # Sales chart
        sales_data = (
            OrderItem.objects.values("order__order_date__month")
            .annotate(total_revenue=F("quantity") * F("unit_price"))
            .order_by("order__order_date__month")
        )

        sales_rev_labels = [
            f'Month {item["order__order_date__month"]}' for item in sales_data
        ]
        ...
        customer_orders_labels = [
            f"{item['order__customer__first_name']} {item['order__customer__last_name']}"
...
def customer_view(request):
    ...
    return render(request, "inventorymanager/customers.html", {"customers": customers})
...
def product_view(request):
    ...
    if low_stock_count > 0:
        if low_stock_count > 1:
            messages.error(request, f"{low_stock_count} items have low stock")
        else:
            messages.error(request, f"{low_stock_count} item has low stock")
...
def order_view(request):
    ...
    return render(request, "inventorymanager/orders.html", {"orders": orders})
...
def create_order(request):
    ...
    return render(
        request,
        "inventorymanager/createOrder.html",
        {
...
def customer_detail(request, id):
    ...
    return render(
        request, "inventorymanager/customer_detail.html", {"customer": customer}
    )
...
def order_detail(request, id):
    ...
    OrderItemFormSet = inlineformset_factory(
        Order, OrderItem, form=OrderItemForm, extra=0, can_delete=False
    )
